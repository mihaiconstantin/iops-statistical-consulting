...
## Research question
The goal of the current project is to assess the accuracy, i.e., the sensitivity and specificity, of each of the four tests used in the clinical trial. The challenge is that none of the methods yields a perfect diagnosis. Therefore, the accuracy needs to be estimated from the data.

## Estimating Test Sensitivity and Specificity through Latent Class Analysis

### A Simple Model
In line with previous research (e.g., Clements et al., 2018), we use Latent Class Analysis (LCA), to answer the research question. LCA is a statistical modeling technique that can infer test sensitivity and specificity as well as disease prevalence from the test result patterns observed in the data. Below, we show a depiction of the simplest version of an LCA model (figure adapted from [Helman et al. (2019)](https://doi.org/10.1002/ece3.6448).

<center>
    ![](../img/model_generic.jpg){width=60%}
</center>
...
#### Trace Results
In the CCA method, test results are categorized as positive, negative, or trace results. The latter category contains results that are ambiguous with respect to the disease status. With LCA requiring dichotomous test outcomes, trace results need to be classified as either positive or negative. Depending on this decision, the counts for obtained test patterns will change, and so will the estimates of test sensitivity and specificity. It is therefore sensible to conduct all analyses twice: once for a trace-positive classification (i.e., trace results are considered positive test outcomes), and once for a trace-negative classification (i.e., trace results are considered negative test outcomes). In this way, estimates for CCA test characteristics can be obtained for both cases, and the robustness of other estimates towards the CCA result classification can be checked.
...
The simple model is based on the assumption of *conditional independence*. This means that the probability to obtain a certain test result pattern, say, `1100`, is only dependent on the prevalence, as well as on the sensitivity and specificity of the tests. In other words, if we know that a person obtained a positive result in test 1, this does not give us any additional information about their probability to obtain a positive result in test 2. However, this is a strong assumption that might not hold in practice.

Let us imagine that the first two tests are identical, i.e., the results of the first test for each individual were copied to obtain the result of test 2. Then, a positive result in the first test would perfectly predict a positive result in the second test, no matter what the underlying test sensitivity or specificity of test are. Of course, in practice, nobody would copy test results. However, two tests can similar to one another independent of their sensitivity and specificity. For example, antigen tests might be more likely to classify participants as infected who were infected in the past, independent of their current disease status. On the other hand, microscopy methods might not be influenced by the past disease status of a participant. Therefore, it can be assumed that two antigen-based methods yield more similar results than an antigen-based method and a microscopy-based method. This dependency can be represented in the statistical model by adding parameters for test dependence.
...
<center>
    ![](../img/informed_priors.jpg)
</center>

### Data Preparation

#### Required R-Packages
To conduct the analyses, you need to install several R-packages.
...
For the analyses described below, we use pseudo-results based on simulated data.
Such simulated data can be obtained using the function `sim.LCA()` located at `./scripts/helpers/simulating.R`.
To load this function you can source the file mentioned above as:

```{r results='hide', message=FALSE}
source("./scripts/helpers/simulating.R")
```

#### Using Real Data

To run the analyses described below, your data needs to be in a pattern-count table (see figure above).
When using simulated data, the data is already provided to you in this format, and can readily be plugged into the models.
However, when using real data, there are a few pre-processing steps that are required, namely:

1. ensure the raw data is in a wide format and the columns are labeled accordingly
2. convert the data from raw test scores to cutoff values
3. use the cutoff values to create the pattern-count table

We provide two functions for performing the data pre-processing steps required to run the analyses.
To illustrate how these functions work, we use a toy data set containing 20 respondents and four tests (i.e., KK, CCA, CAA, and PCR) at four time points (i.e., week 1, 2, 3, and 4).
You can load and inspect this toy data set using:
...
Your real data may contain many respondents, but this toy data set only contains 20 respondents.
However, it is very important that your real data set follows the same structure as this toy data set.
This means that:

- the first two columns should contain the `id` and the `group` variables
- each next four columns should contain the four tests at a specific time point 1 (e.g., KK, CCA, CAA, and PCR at time point 1)
- the ordering of the tests should be the same across all time points (i.e., preferably KK, CCA, CAA, and PCR)
- the first two columns should be named `id` and `group`
- the remainder of the columns should be named using the following pattern: "test_timepoint", e.g.:
    - kk_t1 -> for KK at time point 1
    - cca_t1 -> for CCA at time point 1
    - kk_t2 -> for KK at time point 2
    - and so on...
- preferably, all columns names should be in lowercase

Assuming that all four tests are applied at four different measurement occasions, the names of the columns should look like:

```{r}
colnames(data.toy)
```

Once the real data is in the expected format, the first pre-processing step consists of determining the cutoffs.
To do this, we can use the function `apply.cutoff()` located at `./scripts/helpers/preprocessing.R` as:

```{r}
source("./scripts/helpers/preprocessing.R")

# Determine the cutoffs for the raw data.
...
The argument `trace` of `apply.cutoff()` indicates what cutoff value should be applied to the raw scores that were identified as *trace*.
Currently, this argument only applies for the CCA test.

The final step consists of using the cutoff values to create the pattern-count table.
To do this, we use the function `count.patterns()` located at `./scripts/helpers/preprocessing.R`.
This function takes three arguments:

1. the cutoff values determined above (i.e., `cutoff.data()`)
2. a specific time point for which to perform the count (e.g., time point one specified as `t1`)
3. a specific ordering of the tests; this is needed later when taking into account the dependencies between each pair of tests (i.e, default ordering is KK, CCA, CAA, and PCR)

The function `count.patterns()` computes the frequencies taking into account each treatment group.
It can be used as:

```{r}
# Compute frequencies.
pattern.data <- count.patterns(cutoff.data, timepoint = "t1", tests.ordering = c("kk", "cca", "caa", "pcr"))
...
- the first four columns represent the tests, in the order indicated by the `tests.ordering` argument
- the 16 rows indicate all possible combinations of response patterns
- columns `g1_t1` and `g2_t1` hold the frequencies for each response pattern at time point one, for the first group (i.e., `g1`), and second group (i.e., `g2`) respectively
- column `t1` indicates the frequency for each response pattern at time point one, regardless of the treatment group

Ultimately, the pattern-count table (e.g., `pattern.data`) is needed to run the analyses below.
...
# Note.
# The object `sim` below contains simulated data.
...
We can see one posterior distribution for each parameter. Looking at the posterior distributions gives us an idea about the (a)symmetry of the distribution and about good ways to summarize the posterior. For example, an often-reported value is the mean of the posterior distribution, but for a highly asymmetric distribution it might also be relevant to report the median or mode. For example, for the posterior distributions above, the mean of the posterior distribution is a better representation for the sensitivity parameters, while the median of the posterior distribution is a better representation for the specificity parameters, since the posterior distributions of the specificity parameters have a strong negative skew.

```{r}
MCMCplot(samples)
```

The next plot (see above) shows the posterior means and 95\% credible intervals for each parameter. The width of the credible interval indicates the uncertainty about the parameter estimate.
...
The table shows for each parameter the mean (mu.vect), the standard deviation (sd.vect), as well as five quantiles of the posterior distribution. It also contains the R-hat value and an effective sample size for convergence diagnostics (this is related to the samples from the posterior distribution, *not* the number of participants in the study). The effective sample size measure reported in the table also refers to the number of samples from the posterior distribution, and is a measure of convergence closely related to the trace plots. If there is little autocorrelation in the samples (as it should be), the trace plots look like a hairy caterpillar, and the effective sample size reported in this table close to the actual number of samples drawn from the posterior distribution.
...
# Note.
# The objects `group1sim` and `group2sim` below contain simulated data.
# Please replace it with your own data.
...
If you want to change the order of the tests in your result pattern table to allow different tests to be dependent, use the `count.patterns()` function described above, specifying the appropriate ordering of the tests using the `test.ordering` argument. We recommend running Model 3 with all different combinations of tests (6 combinations overall), such that each time to different tests are assumed to be dependent, and check the robustness of sensitivity and specificity estimates to the test dependency.
...
### Recommendations for Reporting and Conclusions
Fitting all of the statistical models described above will yield a large number of estimates for the sensitivity and specificity of each test for each measurement occasion. For reporting the results, we recommend focusing on one of the models that seems most realistic from a substantive point of view. Results of the other models can be reported as robustness analyses in an appendix. The article by Clements et al. (2018) provides a good example for reporting the results of multiple analyses in a concise manner. Since the focus of the study is on parameter estimation for test sensitivity and specificity, we recommend to report measures of uncertainty for the estimates. In Bayesian modeling, this can be done using credible intervals, i.e., the range of parameter values covering 95\% of the posterior distribution. These can be obtained from the results tables using the 2.5\% and 97.5\% quantile of the posterior distribution.

Although not the main focus of this study, the proposed analyses also allow preliminary conclusions about the treatment effectiveness. Since prevalence is assessed at each measurement occasion, it is possible to monitor prevalence estimates over the course of time, and descriptively check whether there is a decrease in prevalence. Note, however, that our models do not account for dependency of single data points due to repeated measurements. This means that the models do not "know" that the participants in the samples at different measurement occasions are the same. Therefore, observed trends in prevalence should be taken with a grain of salt.
